...
[근거 규칙]
- 제공된 '근거' 안의 내용만 사용한다. 근거에 없으면 추측하지 말고 [확인 불가]로 표기한다. 존재하지 않는 조문·문단번호·판례(심판례)번호·링크를 지어내지 않는다.
- [확인 불가]는 '결론에 직접 영향을 주는 조문·수치·요건'이 근거에 없을 때만 쓴다. 심판례·판례가 근거에 없다는 이유로 별도의 [확인 불가] 문단을 만들지 말고, 그런 경우 '실무 유의'에 "사안별 판단을 위해 관련 심판례·판례의 추가 조회를 권고" 정도로 한 줄만 덧붙인다. 근거에 이미 관련 조문이 있으면 그 조문으로 충분히 결론을 내고, 다른 세목(예: 법인 vs 개인) 조문이 없다는 식의 군더더기 [확인 불가]는 넣지 않는다.
- 근거 유형별 인용법:
...
async function compose(
  ...
  key: string,
  model: string,
  refine?: { priorAnswer: string; followup: string },
): Promise<string> {
  const grounding = groundingBlock || '(제공된 근거 없음 — 본문에서 [확인 불가] 처리)';
  const user = refine
    ? `다음은 기존 회신 초안이다. 아래 '보완 요청'을 반영하여 '근거' 범위 안에서 회신을 개정하라. 기존 초안의 좋은 부분은 유지하되, 보완 요청을 반영해 전체를 다시 '완결된 회신'으로 출력한다(발췌·차이만 내지 말 것).
...
[근거 — 이 범위 안에서만 인용]
${grounding}`
    : `다음 질문에 대해, 아래 '근거' 안의 내용만 사용하여 완성된 이메일 회신을 작성하라.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isRefine = typeof followup === 'string' && followup.trim().length > 0;
    const groundingQuery = (isRefine ? `${question}\n\n[보완 요청] ${followup}` : question).trim();
    // 분야별 근거 조회: 회계 → 회계기준만, 세무 → 세법만, 공통 → 둘 다
    const doAccounting = domain !== '세무';
    const doTax = domain !== '회계';

    // 질의 임베딩 — 회계기준(회계·공통)과 자료실(공통) 근거 검색에 함께 쓴다.
    const qe = await embed(groundingQuery, openaiKey);
    ...
    // 3) Claude로 회신 초안 작성/개정 (선택 모델)
    const answer_md = await compose(
      question.trim(), groundingBlock, anthropicKey, useModel,
      isRefine ? { priorAnswer: String(priorAnswer), followup: String(followup) } : undefined,
    );
