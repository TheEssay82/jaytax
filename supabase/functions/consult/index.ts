...
async function compose(question: string, groundingBlock: string, key: string): Promise<string> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { question, standardNo = '1115', matchCount = 6, lawRefs = [] } = await req.json().catch(() => ({}));
    ...
    // 3) Claude로 회신 초안 작성
    const answer_md = await compose(question.trim(), groundingBlock, anthropicKey);

    return json({ ok: true, answer_md, citations, model: CLAUDE_MODEL });
  } catch (e) {
