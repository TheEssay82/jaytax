...
drop function if exists public.essay_submit_ranking(uuid, uuid[], text);
drop function if exists public.essay_ranking_sheet(uuid);
drop function if exists public.essay_mark_read(uuid, uuid);
...
drop table if exists public.essay_ranking;
drop table if exists public.essay_read;
