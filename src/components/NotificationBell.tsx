        ...
        {unread > 0 && (
          <span
            style={{
              ...
              background: '#dc2626', color: '#fff', borderRadius: 8, fontSize: 'var(--fs-0)', fontWeight: 700,
      ...
      {open && (
        <div
          style={{
            ...
            border: '1px solid var(--rule)', borderRadius: 10, boxShadow: '0 8px 28px rgba(0,0,0,0.16)',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', padding: '9px 12px', borderBottom: '1px solid var(--rule-2)', position: 'sticky', top: 0, background: '#fff' }}>
            <b style={{ fontSize: 'var(--fs-2)', color: 'var(--navy)' }}>알림{unread > 0 ? ` · ${unread}` : ''}</b>
            <button
              className="btn-sm"
              style={{ marginLeft: 'auto', fontSize: 'var(--fs-0)', padding: '2px 7px' }}
              disabled={unread === 0}
          ...
          {permAsk && (
            <div style={{ padding: '8px 12px', background: '#FFF9E8', borderBottom: '1px solid #F0DFA8', fontSize: 'var(--fs-1)', color: '#8a5a00' }}>
              💡 다른 창을 보고 있어도 알림을 받으려면{' '}
              <button className="btn-sm" style={{ fontSize: 'var(--fs-0)', padding: '1px 7px', marginLeft: 2 }} onClick={() => void enablePush()}>
                알림 켜기
              </button>
            </div>
          )}
          ...
          {items.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--ink-4)', fontSize: 'var(--fs-2)' }}>알림이 없습니다.</div>
          ) : (
            items.map((n) => (
              <button
                key={n.id}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                  ...
                  <b style={{ fontSize: 'var(--fs-2)', color: 'var(--navy)' }}>{n.title}</b>
                  {!n.readAt && <span style={{ width: 6, height: 6, borderRadius: 3, background: '#dc2626', marginLeft: 2 }} />}
                  <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-0)', color: 'var(--ink-3)', whiteSpace: 'nowrap' }}>{dt(n.createdAt)}</span>
                </div>
                {n.body && <div style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-2)', marginTop: 2, lineHeight: 1.35 }}>{n.body}</div>}
              </button>
            ))
          )}
        </div>
