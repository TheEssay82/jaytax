...
export default function LawRefPicker({
  ...
  return (
    <div style={{ border: '1px solid var(--rule)', borderRadius: 8, padding: '12px 14px', background: '#faf8f3' }}>
      {/* 선택된 근거 칩 */}
      {value.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
          ...
          <button type="button" className="btn-sm" onClick={() => onChange([])} style={{ fontSize: 'var(--fs-1)' }}>모두 비우기</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
        {TAX_LAW_QUICKLIST.slice(0, 7).map((name) => (
          <button key={name} type="button" className="btn-sm" onClick={() => { setQuery(name); run(name); }} disabled={busy} style={{ fontSize: 'var(--fs-1)' }}>
            {name}
          </button>
        ))}
      </div>

      {error && <div className="alert-w" style={{ marginTop: 10, fontSize: 'var(--fs-2)' }}>{error}</div>}
      {detailBusy && <div className="alert-i" style={{ marginTop: 10, fontSize: 'var(--fs-2)' }}>조문을 불러오는 중…</div>}

      {/* 법령 목록 */}
      {laws && !detail && !detailBusy && (
        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 220, overflowY: 'auto' }}>
          {laws.length === 0 ? (
            <div className="alert-i" style={{ fontSize: 'var(--fs-2)' }}>검색 결과가 없습니다.</div>
          ) : (
            laws.map((l) => (
              <button key={l.mst} type="button" onClick={() => openDetail(l)} style={lawRowStyle}>
                <span style={{ fontWeight: 700, color: 'var(--navy)', fontSize: 'var(--fs-3)' }}>{l.name}</span>
                {l.lawType && <span className="bdg b-on" style={{ fontSize: 9 }}>{l.lawType}</span>}
                <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-1)', color: 'var(--ink-3)' }}>시행 {fmtEffDate(l.effDate)}</span>
              </button>
            ))
          )}
        </div>
      ...
      {detail && (
        <div style={{ marginTop: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 'var(--fs-3)', fontWeight: 700, color: 'var(--navy)' }}>{detail.name}</span>
            <span style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-3)' }}>시행 {fmtEffDate(detail.effDate)} · 조문 {detail.articleCount}개</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, maxHeight: 300, overflowY: 'auto' }}>
            {detail.articles.map((a, i) =>
              a.isChapter ? (
                <div key={i} style={{ fontSize: 'var(--fs-2)', fontWeight: 700, color: 'var(--ink-3)', margin: '8px 0 2px' }}>{a.content}</div>
              ) : (
                (() => {
                      ...
                      <span style={{ flex: 1 }}>
                        <span style={{ fontSize: 'var(--fs-2)', fontWeight: 700, color: 'var(--navy)' }}>
                          ...
                          {a.effDate && a.effDate !== detail.effDate && (
                            <span style={{ fontWeight: 400, fontSize: 'var(--fs-0)', color: 'var(--ink-3)' }}> · 시행 {fmtEffDate(a.effDate)}</span>
                          )}
                        </span>
                        <span style={{ display: 'block', fontSize: 'var(--fs-2)', lineHeight: 1.55, color: '#4b5563', whiteSpace: 'pre-wrap', marginTop: 1 }}>
      ...
      <div style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-3)', marginTop: 10, lineHeight: 1.55 }}>
...
const chipStyle: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 'var(--fs-1)', fontWeight: 600,
  color: 'var(--navy)', background: '#fff', border: '1px solid #d9b25f', borderRadius: 14, padding: '3px 6px 3px 10px',
};
const chipX: React.CSSProperties = {
  border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, lineHeight: 1, color: 'var(--ink-3)', padding: '0 2px',
};
const lawRowStyle: React.CSSProperties = {
  textAlign: 'left', border: '1px solid var(--rule)', borderRadius: 6, padding: '7px 10px',
  background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, width: '100%',
};
const artRowStyle: React.CSSProperties = {
  display: 'flex', gap: 8, alignItems: 'flex-start', border: '1px solid var(--rule-2)', borderRadius: 6,
