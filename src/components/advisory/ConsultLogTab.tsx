    ...
    <div className="card">
      ...
      {topTags.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', marginTop: 10 }}>
          ...
          {tagFilter && (
            <button className="btn-sm" style={{ fontSize: 'var(--fs-1)' }} onClick={() => setTagFilter(null)}>필터 해제</button>
          )}
        </div>
      )}
      ...
      {items !== null && (
        <div style={{ fontSize: 'var(--fs-2)', color: 'var(--ink-3)', margin: '10px 0' }}>
          ...
          {clientFilter && (
            <> · 거래처 <b>{clientFilter}</b> <button className="btn-sm" style={{ fontSize: 'var(--fs-1)' }} onClick={() => setClientFilter('')}>해제</button></>
          )}
        </div>
      )}
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
        ...
        {seq != null && (
          <span style={{ fontSize: 'var(--fs-2)', fontWeight: 700, color: '#1A6E3C' }}>확정 #{seq}</span>
        )}
        ...
        {editing ? (
          <input
            ...
            style={{ flex: 1, minWidth: 240, fontSize: 'var(--fs-4)', fontWeight: 700 }}
          />
        ) : (
          <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--navy)' }}>{item.title || '(제목 없음)'}</span>
        )}
      </div>
      <div style={{ fontSize: 'var(--fs-2)', color: 'var(--ink-3)', marginBottom: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        {isOwner && !editing && (
          <button className="btn-sm" onClick={remove} disabled={busy} style={{ color: 'var(--bad)' }}>🗑️ 삭제</button>
        )}
        {isOwner && editing && (
          <>
      ...
      <Section label="질문 · 사실관계">
        <div style={{ fontSize: 'var(--fs-3)', lineHeight: 1.65, color: 'var(--ink-2)', whiteSpace: 'pre-wrap' }}>{item.question}</div>
      </Section>
        ...
        <Section label={`근거 (${item.citations.length})`}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {item.citations.map((c, i) => (
              <div key={i} style={{ border: '1px solid var(--rule)', borderRadius: 8, padding: '10px 12px', background: '#fffdf6' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4, flexWrap: 'wrap' }}>
                  <span className="bdg" style={{ fontSize: 'var(--fs-0)', color: c.type === '세법' ? '#1A2B52' : '#8a5a00' }}>{c.type}</span>
                  <span style={{ fontSize: 'var(--fs-2)', fontWeight: 700, color: 'var(--navy)' }}>{c.ref}</span>
                </div>
                <div style={{ fontSize: 'var(--fs-2)', lineHeight: 1.6, color: '#4b5563', whiteSpace: 'pre-wrap' }}>{c.text}</div>
              </div>
            ))}
          </div>
        </Section>
      ...
      <div style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-3)', marginTop: 12, lineHeight: 1.6 }}>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 'var(--fs-3)', fontWeight: 700, color: 'var(--navy)', marginBottom: 6 }}>{label}</div>
      {children}
    </div>
  );
}
...
function StatusBadge({ status }: { status: ConsultStatus }) {
  ...
  return (
    <span
      className="bdg"
      style={{
        fontSize: 'var(--fs-0)', fontWeight: 700,
  ...
  return (
    <div style={{ border: '1px dashed #d8d2c6', borderRadius: 8, background: '#fbfaf6', padding: '10px 12px', marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 'var(--fs-2)', fontWeight: 700, color: 'var(--navy)' }}>🔗 외부 공유</span>
        <span style={{ fontSize: 'var(--fs-1)', color: shared ? '#1A6E3C' : '#9aa0ad' }}>
      ...
      {shared && (
        <div style={{ display: 'flex', gap: 6, marginTop: 8, alignItems: 'center' }}>
          <input readOnly value={url} onFocus={(e) => e.target.select()} style={{ flex: 1, fontSize: 'var(--fs-2)', fontFamily: 'monospace' }} />
...
function ClientBadge({ name }: { name: string }) {
  return (
    <span
      className="bdg"
      style={{
        fontSize: 'var(--fs-0)', fontWeight: 700, color: 'var(--navy)',
