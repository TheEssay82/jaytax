  ...
  return (
    <div style={{ border: '1px solid #cdd8c0', borderRadius: 8, background: '#f6faef', padding: '8px 10px', marginBottom: 12 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 6 }}>
        <b style={{ fontSize: 12.5, color: '#465' }}>📋 예산 (차기 정산연도 projection)</b>
        ...
        <span style={{ fontSize: 10.5, color: '#8a6' }}>확정분=등록계약 projection(자동) · 갱신분={year - 1}귀속 연단위 계약을 계속 가정 · 공급가액(순액)</span>
        {msg && <span style={{ marginLeft: 'auto', fontSize: 11, color: '#2a7' }}>{msg}</span>}
        {!canEdit && <span style={{ fontSize: 10.5, color: '#b95' }}>· 편집은 회계사·팀장·최고관리자</span>}
      </div>

      {/* 팀별 예산 요약 */}
      <table style={{ borderCollapse: 'collapse', fontSize: 12, marginBottom: 10, minWidth: 460 }}>
      ...
      {/* 갱신 후보 편집 */}
      <div style={{ fontSize: 11, fontWeight: 700, color: '#576', margin: '2px 0 4px' }}>갱신 후보 — {year - 1}귀속 연단위 계약 ({rows.length}건) {busy && <span style={{ color: '#999' }}>· 저장 중…</span>}</div>
      {rows.length === 0 ? (
        <div style={{ fontSize: 11, color: '#999' }}>{year - 1}귀속 연단위 계약이 없습니다.</div>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.c.id} style={{ borderTop: '1px solid #e0e8d2', opacity: r.active ? 1 : 0.5 }}>
                  ...
                  <td style={{ ...tdc, textAlign: 'right', color: '#999' }}>{won(r.priorNet)}</td>
                  <td style={{ ...tdc, textAlign: 'right' }}>
                    {canEdit ? (
                      <input type="text" defaultValue={won(r.amount)} disabled={busy || !r.active}
                        ...
                        style={{ width: 96, textAlign: 'right', fontSize: 11.5, padding: '2px 4px' }} />
                    ) : won(r.amount)}
                  </td>
                </tr>
              ))}
