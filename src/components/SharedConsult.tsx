  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f2ec', padding: '24px 16px' }}>
      <div style={{ maxWidth: 780, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, color: 'var(--navy)' }}>
          <span style={{ fontWeight: 800, letterSpacing: 1 }}>JAY</span>
          <span style={{ fontSize: 'var(--fs-2)', color: 'var(--ink-3)' }}>세무회계 지원 · 상담 회신</span>
        </div>
        ...
        {state === 'notfound' && (
          <div style={card}>
            ...
            <div style={{ fontSize: 'var(--fs-3)', color: 'var(--ink-3)', marginTop: 6 }}>작성자가 공유를 해제했을 수 있습니다.</div>
          </div>
        )}
        ...
        {state === 'ok' && data && (
          <div style={card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 5 }}>
              <span style={{ fontSize: 'var(--fs-0)', fontWeight: 700, color: data.status === 'final' ? '#1A6E3C' : '#8a5a00', background: data.status === 'final' ? '#e6f4ec' : '#fdf3e0', border: `1px solid ${data.status === 'final' ? '#bfe3cc' : '#f0dcb4'}`, borderRadius: 4, padding: '2px 8px' }}>
                {data.status === 'final' ? '확정' : '초안'}
              </span>
              <span style={{ fontSize: 'var(--fs-5)', fontWeight: 800, color: 'var(--navy)', letterSpacing: '-0.01em' }}>{data.title || '(제목 없음)'}</span>
            </div>
            <div style={{ fontSize: 'var(--fs-2)', color: 'var(--ink-3)', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {data.citations.map((c, i) => (
                    <div key={i} style={citeBox}>
                      ...
                      <div style={{ fontSize: 'var(--fs-2)', lineHeight: 1.7, color: '#5a6270', whiteSpace: 'pre-wrap' }}>{c.text}</div>
                    </div>
                  ))}
                </div>
              </Section>
            ...
            <div style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-3)', marginTop: 20, lineHeight: 1.6, borderTop: '1px solid var(--rule-2)', paddingTop: 12 }}>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontSize: 'var(--fs-1)', fontWeight: 700, color: '#a89b80', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 8 }}>{label}</div>
      {children}
    </div>
  );
}
...
const questionBox: React.CSSProperties = {
  ...
  background: '#f8f6f1', border: '1px solid var(--rule-2)', borderRadius: 10, padding: '13px 15px',
};
const citeBox: React.CSSProperties = {
  border: '1px solid var(--rule-2)', borderLeft: '3px solid #c9b88a', borderRadius: '4px 8px 8px 4px', padding: '11px 14px', background: '#fffdf8',
};
