    ...
    <div style={{ marginBottom: 12 }}>
      ...
      <div className="tbl-scroll" style={{ maxHeight: '56vh' }}>
        <table className="tbl" style={{ fontSize: 11.5 }}>
          ...
          <tbody>
            {busy && (
              <tr><td colSpan={11 + BUCKETS.length} style={{ textAlign: 'center', padding: 20, color: '#BBB' }}>
                계산 중…
              </td></tr>
            )}
            ...
            {!busy && sorted.map((r) => (
              <tr key={r.placeId} style={{ background: r.overdue > 0 ? '#FFF7ED' : undefined }}>
                ...
                <td style={{ fontSize: 11 }}>{r.staff || <span style={{ color: '#CCC' }}>—</span>}</td>
                {BUCKETS.map((b) => {
                ...
                <td style={{ fontSize: 11 }}>
                  {r.overdue > 0
                    ? (r.notified ? <span style={{ color: '#2a7' }}>✓ 보냄</span> : <span style={{ color: '#C99' }}>아직</span>)
      ...
      <div className="card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 640, width: '100%' }}>
        ...
        <div style={{ maxHeight: '52vh', overflow: 'auto' }}>
          <table className="tbl" style={{ fontSize: 11.5 }}>
            ...
            <tbody>
              {row.items.length === 0 && (
                <tr><td colSpan={4} style={{ textAlign: 'center', padding: 16, color: '#BBB' }}>
