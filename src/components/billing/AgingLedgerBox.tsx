  ...
  return (
    <div style={{ border: '1px solid #c9b98a', borderRadius: 8, background: '#fdfaf3', padding: '8px 10px', marginBottom: 10 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <b style={{ fontSize: 12.5, color: '#1A2B52' }}>📒 미수금대장</b>
        {up ? (
          <span style={{ fontSize: 11.5, color: '#2a7' }}>
            {ym} {teamLabel(team)} 올림 — {up.rowCount}건 · 잔금 {won(up.balanceTotal)}
            {' · '}{up.uploadedAt.slice(0, 10)}{up.uploadedBy && ` ${up.uploadedBy}`}
          </span>
        ) : (
          <span style={{ fontSize: 11.5, color: '#a15' }}>
      ...
      <div style={{ fontSize: 10.5, color: '#998', marginTop: 3 }}>
        인덕 ERP ▸ <b>기간 미수금대장</b> ▸ 기간 {ym}-01 ~ 말일 ▸ 조회 ▸ 엑셀.
        건별 invoiceNo(전표번호)에 <b>발행일</b>이 들어 있어, 이걸 올리면 미수금 나이를 추정 없이 잽니다.
      </div>

      {preview && (
        <div style={{ marginTop: 8, borderTop: '1px solid #e2d9c6', paddingTop: 8 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: '#1A2B52' }}>
            읽었습니다 — {preview.rows.length}건 · 잔금 {won(preview.balanceTotal)}
          </div>
          <div style={{ fontSize: 11.5, color: '#666', margin: '4px 0 8px', lineHeight: 1.7 }}>
      ...
      {unmatched.length > 0 && (
        <div style={{ marginTop: 8 }}>
          ...
          {open && (
            <>
              <div style={{ fontSize: 10.5, color: '#998', margin: '4px 0' }}>
                  ...
                  <tbody>
                    {unmatched.map((u) => (
                      <tr key={u.clientName} style={{ opacity: u.excluded ? 0.5 : 1 }}>
                        <td style={{ fontWeight: 700, color: '#1A2B52' }}>{u.clientName}</td>
                        <td>{u.cpa}</td>
