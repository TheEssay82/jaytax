...
const stageStyle = (s: string): React.CSSProperties => {
  ...
  if (s === '반송 있음') return { background: '#FEE2E2', color: 'var(--bad)' };
  ...
  return (
    <div className="card">
      ...
      <div className="sbar">
        ...
        <span style={{ fontSize: 'var(--fs-1)', color: 'var(--ink-3)' }}>발송 {totals.sent}건</span>
        <span style={{ fontSize: 'var(--fs-1)', color: '#065F46', fontWeight: 700 }}>회수 {totals.collected}/{totals.sent}</span>
        {totals.returned > 0 && (
          <span style={{ fontSize: 'var(--fs-1)', color: 'var(--bad)', fontWeight: 700 }}>반송 {totals.returned}</span>
        )}
        ...
        <span style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
          <button
            className="btn-sm"
            style={{
              fontSize: 'var(--fs-1)', padding: '2px 8px', fontWeight: 700,
              ...(mode === 'overdue'
      ...
      <div className="tbl-scroll">
        <table className="tbl">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--ink-4)', padding: 24 }}>
    ...
    <>
      <tr style={open ? { background: '#EEF6FF' } : undefined}>
        <td style={{ textAlign: 'center' }}>
          <button className="btn-sm" style={{ fontSize: 'var(--fs-0)', padding: '1px 5px' }} onClick={onToggle}>
            {open ? '▾' : '▸'}
          </button>
        </td>
        ...
        <tr>
          <td colSpan={7} style={{ background: '#F7FAFF', padding: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
              ...
              <button className="btn-sm" style={{ fontSize: 'var(--fs-1)' }} disabled={busy || sel.size === 0} onClick={() => setSel(new Set())}>
                해제
              </button>
              <span style={{ fontSize: 'var(--fs-1)', fontWeight: 700, color: 'var(--navy)' }}>{selected.length}건 선택</span>
              <button
                className="btn-p"
                style={{ fontSize: 'var(--fs-1)' }}
              ...
              <button
                className="btn-sm"
                style={{ fontSize: 'var(--fs-1)', background: '#FEE2E2', color: 'var(--bad)', fontWeight: 700 }}
                disabled={busy || selected.length === 0}
                onClick={() => void bulkCollect('반송')}
              >
                ↪ 반송 (사유 입력)
              </button>
              <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-1)', color: 'var(--ink-2)' }}>미처리 {pending.length}건</span>
            </div>
            ...
            <table className="tbl">
              ...
              <tbody>
                {sentItems.length === 0 && (
                  <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--ink-4)', padding: 16 }}>
            ...
            {items.some((i) => !i.sent) && (
              <div style={{ fontSize: 'var(--fs-0)', color: '#8a5a00', marginTop: 6 }}>
  ...
  return (
    <tr style={done ? { background: '#F6FBF7' } : returned ? { background: '#FFF7F7' } : undefined}>
      ...
      <td style={{ fontSize: 'var(--fs-1)' }}>
        ...
        {!done && !returned && <span style={{ color: 'var(--ink-4)' }}>미처리</span>}
      </td>
      <td style={{ textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: 3, justifyContent: 'center' }}>
          {it.collectStatus === null ? (
            <>
              ...
              <button
                className="btn-sm"
                style={{ fontSize: 'var(--fs-0)', background: '#FEE2E2', color: 'var(--bad)' }}
                disabled={busy}
            ...
            <>
              {returned && (
                <button className="btn-sm" style={{ fontSize: 'var(--fs-0)' }} disabled={busy} onClick={markReturned} title="반송 사유 수정">
                  ✏️
                </button>
              )}
              <button
...
function OverdueView({
  ...
  return (
    <>
      ...
      {rows.length > 0 && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
          <button className="btn-sm" style={{ fontSize: 'var(--fs-1)' }} disabled={busy} onClick={() => setSel(new Set(rows.map((r) => r.item.id)))}>
            전체선택 ({rows.length})
          </button>
          <button className="btn-sm" style={{ fontSize: 'var(--fs-1)' }} disabled={busy || sel.size === 0} onClick={() => setSel(new Set())}>해제</button>
          <span style={{ fontSize: 'var(--fs-1)', fontWeight: 700, color: 'var(--navy)' }}>{sel.size}건 선택</span>
          <button className="btn-p" style={{ fontSize: 'var(--fs-1)' }} disabled={busy || sel.size === 0} onClick={() => void bulk('회수완료')}>
            ✅ 회수완료
          </button>
          <button
            className="btn-sm"
      ...
      <div className="tbl-scroll">
        <table className="tbl">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={11} style={{ textAlign: 'center', color: '#7A9B7A', padding: 24, fontSize: 'var(--fs-2)' }}>
            ...
            {rows.map(({ conf, item, days: d }) => (
              <tr key={item.id}>
                ...
                <td style={{ textAlign: 'center', fontSize: 'var(--fs-2)', ...dayStyle(d) }}>{d}일</td>
                <td style={{ fontSize: 'var(--fs-2)' }}><b style={{ color: 'var(--navy)' }}>{conf.companyName}</b></td>
                <td style={{ fontSize: 'var(--fs-1)' }}>{conf.accountantName || <span style={{ color: 'var(--ink-4)' }}>—</span>}</td>
                <td style={{ fontSize: 'var(--fs-1)' }}>{item.kind}</td>
                <td style={{ fontSize: 'var(--fs-2)' }}><b>{item.institution}</b></td>
                <td style={{ textAlign: 'center' }}>
                  <span
                    className="bdg"
                    style={{
                      fontSize: 'var(--fs-0)',
                      ...(item.isElectronic ? { background: '#DBEAFE', color: '#1E40AF' } : { background: '#FEF3C7', color: 'var(--warn)' }),
                    }}
                  >
                    {item.isElectronic ? '전자조회' : '실물발송'}
                  </span>
                </td>
                <td>{item.isElectronic ? <span style={{ color: 'var(--ink-4)', fontSize: 'var(--fs-1)' }}>—</span> : <TrackingLink no={item.trackingNo} />}</td>
                <td style={{ textAlign: 'center', fontSize: 'var(--fs-1)' }}>{item.sentDate?.replace(/-/g, '.') ?? '—'}</td>
                <td style={{ fontSize: 'var(--fs-1)' }}>{item.phone || <span style={{ color: 'var(--ink-4)' }}>—</span>}</td>
                <td style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', gap: 3, justifyContent: 'center' }}>
                    <button
                      className="btn-sm btn-p"
